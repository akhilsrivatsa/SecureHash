...
1. Size of the work unit that you determined results in the best performance for your implementation and an explanation of how you determined it. The size of the work unit refers to the number of sub-problems that a worker gets in a single request from the boss. 
   ...
   **Ans**. In our system, each worker has the following tasks/sub-problem
    ...
    After testing various design and work distributions between the server and the worker program and an exhaustive trial and error process, we believe for our application which uses TCP connection this is the most optimal structure of the worker program.


2. The result of running your program for input 4

    **Ans**. (temporary answer, until 4 laptops are used)
...
3. The running time for the above is reported by time for the above and report the time.  The ratio of CPU time to REAL TIME tells you how many cores were effectively used in the computation.  If you are close to 1 you have almost no parallelism (points will be subtracted).
    ...
    **Ans**. The ratio of total CPU Runtime / total Real Time we got approximately around 10 
at the time of termination, but the trajectory was increasing and hence it could 
be > 10 depending on the time the program is run.
...
5. The largest number of working machines you were able to run your code with.
   ...
   **Ans**. The no. of machines available to us during testing the application was 5, but it could be more without any issue in performance.
